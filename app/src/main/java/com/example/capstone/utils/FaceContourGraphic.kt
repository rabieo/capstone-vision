...
class FaceContourGraphic(overlay: GraphicOverlay?) : GraphicOverlay.Graphic(overlay!!) {
    ...
    override fun draw(canvas: Canvas?) {
        ...
        val y: Float = translateY(face.boundingBox.centerY().toFloat()) +270f
        ...
        for (faceContour in contour) {
            for (point in faceContour.points) {
                ...
                val py: Float = translateY(point.y) +270f
                canvas.drawCircle(px, py, FACE_POSITION_RADIUS, facePositionPaint)
            }
        }
        if (face.smilingProbability == null) {
            canvas.drawText(
                    "x: " + String.format("%.2f", face.boundingBox.centerX().toFloat()),
                    x + ID_X_OFFSET * 3,
                    y - ID_Y_OFFSET,
                    idPaint)
        }
    ...
    companion object {
        ...
        private const val ID_Y_OFFSET = -350.0f
